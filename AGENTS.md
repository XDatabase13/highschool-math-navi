...
数学I「数と式」M1-EC-001〜044（44問、式の計算／因数分解／実数・平方根／一次不等式の4区分）は全問独立検算済みで、公開対象です。教材化・技術実装・build確認まで完了していますが、2026-09-23時点でまだpush・本番deployは行っていません（M1-EC-037〜044は未commit）。詳細は「数と式の現在地」節を参照してください。
