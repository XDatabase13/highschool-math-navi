...
さらにスマホでは、本文やThinkingFlowを読んでいる最中でも画面右上固定の「問題一覧」ボタンから、ネイティブ`<dialog>`で同じ問題一覧をいつでも開けます。問題メタ／解法メタもスマホでは初期状態が閉じた開閉式で、それぞれ独立して開閉できます（PCでは従来どおり常時展開）。
...
開閉にはCSS Grid（`grid-template-rows: 0fr → 1fr` ＋ opacity）による軽いアニメーションを使い、`prefers-reduced-motion`では即時開閉になります。最終解答やスマホの問題メタ／解法メタの開閉も同じ手法です。
